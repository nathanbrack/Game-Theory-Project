...
def random_opponent():
    x = np.random.default_rng().choice([3,4,5,6], p=[0.2,0.3,0.3,0.2])

    if x == 0:
        return Uniform_random(3), "Uniform_random"

    elif x == 1:
        return Fixed_absolute_markovian(3, np.array(
            ([0, 0.5, 0.5], [0.5, 0.0, 0.5], [0.5, 0.5, 0.0]))), "Fixed_absolute_markovian"

    elif x == 2:
        return Fixed_relative_markovian(3, np.array(
            ([1 / 3, 1 / 3, 1 / 3], [1 / 3, 1 / 3, 1 / 3], [0.1, 0.1, .8]))), "Fixed_relative_markovian"
